...
const App = {
    ...
    setupDragDrop(){
        ...
        c.addEventListener('drop',e=>{e.preventDefault();o.classList.remove('active');const f=e.dataTransfer.files[0];if(!f)return;if(f.type.startsWith('audio/'))this.loadAudio(f);else if(f.type.startsWith('image/'))this.loadBgImage(f);});
    },
    setupKeyboard(){
    ...
    loadBgImage(file){const img=new Image();img.onload=()=>{State.bgImage=img;State.bgType='image';Notify.show('Background set','success');};img.src=URL.createObjectURL(file);},
    showPanel(id){
