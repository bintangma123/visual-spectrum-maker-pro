...
const App = {
    ...
    loadBgVideo(file){if(State.bgVideo){State.bgVideo.pause();State.bgVideo.src='';State.bgVideo=null;}const video=document.createElement('video');video.src=URL.createObjectURL(file);video.loop=State.bgVideoLoop;video.muted=State.bgVideoMuted;video.playsInline=true;video.playbackRate=State.bgVideoSpeed;video.addEventListener('canplay',()=>{State.bgVideo=video;State.bgType='video';if(AudioEngine.isPlaying)video.play();Notify.show('Background video loaded','success');},{once:true});video.load();},
    removeBgVideo(){if(State.bgVideo){State.bgVideo.pause();URL.revokeObjectURL(State.bgVideo.src);State.bgVideo.src='';State.bgVideo=null;State.bgType='gradient';Notify.show('Video removed','info');}},
    syncBgVideo(){if(!State.bgVideo)return;if(AudioEngine.isPlaying){if(State.bgVideo.paused)State.bgVideo.play();}else{if(!State.bgVideo.paused)State.bgVideo.pause();}},
    showPanel(id){
